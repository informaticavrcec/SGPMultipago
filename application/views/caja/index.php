...
<form method="post" id="cancelar" action="/caja/cancelar">
...
if(count($actividades_apagar) < 1 ){
	...
	foreach($actividades_apagar as $value){
        ...
        <a href="/caja/descuento/<?=$value['IDPostulacionItem']?>/<?=$value['IDSeccion']?>" class="descuento">descuentos</a>
        </td>
		...
		if($value['cupos'] <= 0){
			...
			?>
            <input type="checkbox" name="a_pagar[]" value="<?=$value['IDPostulacionItem']?>" /><?
		}?></td>
		</tr><?
		$c++;
	}
...
<tr>
<td ></td>
<td width="100" align="center"><input type="button" id="pagar" value="Pagar Selección" /></td>
</tr>
</table>
</form>
<table width="100%" border="0" cellpadding="3" cellspacing="0" align="center" >
