...
class Tz_postulacionitem_tipodepago extends CI_Model {
	...
	function delete_pagos($idpostulacionitem){
		...
		WHERE
		  dbo.tz_grupos_multipago.ids_postulacionitems = '$idpostulacionitem' ");
		
		$array = explode(',',$idpostulacionitem);
		foreach($array as $value){
			$this->db->query("EXEC p_pagos_eliminar $value ");
		}
		return TRUE;	
	}
	...
	function get_procesapago($idpostulacionitem){	
		...
		$query = $this->db->query("SELECT dbo.fn_finalizar_pago2('$idpostulacionitem') AS resp ");
		return $query->row();	
	}
	...
	function get_matricula_alumno($idpostulacionitem){
		...
		foreach($parts as $value){
			$this->db->query("EXEC p_alumno_matricular $value , $id_usuario ");
		}
		return TRUE;	
	}
	...
	public function get_formaspago_porpostulacionitem_boleta($idpostulacionitem,$nro_boleta){
		...
		WHERE
		  ...
		  dbo.tz_IC_Boletas.IDPostulacionItem IN ($idpostulacionitem) AND
	...
	public function get_detalle_delpago($idpostulacionitem,$idtipopago,$numero_boleta = ''){
		...
		switch($idtipopago){
			...
			case 3862 : $sql = "SELECT 
			  ...
			  a.NombreApellido AS creador,
			  b.Nombre AS documento,
			  CAST(z.NroPago AS VARCHAR(100)) + ' <b>/</b> ' + CAST(z.CantPagos AS VARCHAR(100) ) + ' <b>/</b> Nro. Transf : ' + z.NumeroTransferencia + ' <strong>/</strong> Banco : ' + dbo.ta_MetadatoDetalle.nmbre AS detalle,
			  z.Monto,
			  z.IDBanco,
			  z.FechaModificacion AS creado,
			  z.IDTipoDocumento,
			...
			FROM
			  ...
			  LEFT OUTER JOIN dbo.vUsuarioAll ON (z.IDModificador = dbo.vUsuarioAll.idt_usrio)
			WHERE
			  z.IDPostulacionItem IN ($idpostulacionitem) $opt";
			break;
