...
class Caja extends MY_Controller{	
	...
	function imprimirboleta($idpostulacionitem,$numero_boleta = NULL){
		...
		$this->load->helper(array('url','form'));
		$this->load->model(array('Tz_permisos','Tz_grupospago'));
		if($this->Tz_permisos->get_permiso_modulo(4) != TRUE){
			redirect('/accesodenegado');
		}
		...
		if(count($grupos) > 0){
			foreach($grupos as $valor){
				$idpostulacionesitems[] = $valor['IdPostulacionItem'];	
			}
			$idpostulacionesitems = implode(',',$idpostulacionesitems);
		}else{
			$idpostulacionesitems = $idpostulacionitem;
		}
		
		$data['detalleactividad'] = $this->Tz_icboletas->get_imprimir($idpostulacionesitems);
		//echo $idpostulacionesitems; //print_r($data['detalleactividad']);		
		$res = $this->Tz_postulacionitem_tipodepago->get_formaspago_porpostulacionitem_boleta($idpostulacionesitems,$numero_boleta);
	...
	function documento($idpostulacionitem){
		...
		$allow = explode(',',base64_decode($idpostulacionitem));
		asort($allow);		
		foreach($allow as $valor){
			if(!is_numeric($valor)){
		...
		$data['idpostulacionitem'] = base64_encode(end($allow));	
	...
	function procesapago(){
		...
		if($idpostulacionitem){
			...
			$respuesta = $this->Tz_postulacionitem_tipodepago->get_procesapago($idpostulacionitem);		
			if($respuesta->resp == 2){
				
				$parts = explode(',',$idpostulacionitem);
				asort($parts);				
				$this->Tz_grupospago->insert_grupo(end($parts),end($parts));				
				foreach($parts as $value){
					$this->Tz_grupospago->insert_grupo($value,end($parts));	
					//echo $value.' - '.end($parts).'<br>';
				}
				//exit();
	...
	public function eliminarpago($tipo_pago,$idpago,$idpostulacionitem){
		...
		$old = $idpostulacionitem;
		$parts = explode(',',base64_decode($idpostulacionitem));	
		asort($parts);
		$idpostulacionitem = end($parts);		 
		
		$this->load->helper(array('url'));
		$this->load->model('Tz_postulacionitem_tipodepago');
		$this->Tz_postulacionitem_tipodepago->eliminar_pago($tipo_pago,$idpago,FALSE,$idpostulacionitem);
		redirect('/caja/cancelar/'.$old);
	...
	public function cancelar($idpostulacionitem = NULL){
		...
		$this->load->model(array('Tz_permisos','Tz_grupos_multipago'));
		if($this->Tz_permisos->get_permiso_modulo(4) != TRUE){
			redirect('/accesodenegado');
		}
		...
		if($idpostulacionitem){
			...
			if(is_array($valor)){
				...
				$a_pagar = explode(',',$valor);					
			}
		}			
				
		foreach($a_pagar as $numero){						
			if(!is_numeric(trim($numero))){				
				redirect('/caja');	
			}
		}
		$this->Tz_grupos_multipago->update_grupos_multipago($a_pagar,$this->session->userdata('id_usuario'));		
		...
		if($a_pagar){
			...
			foreach($datos_seccion as $valor){	
				...
				if($c == 0){
					$idaplicacion[] = $valor['IDAplicacion'];			
				}
				if($valor['bEstado'] == 3 OR $valor['bEstado'] == 12 OR $valor['bEstado'] == 9 ){
					...
					if(!in_array($valor['IDAplicacion'],$idaplicacion)){
						$data['flash_error'] .= '<li>La actividad '.$valor['Nombre_Curso'].' no puede ser cancelada, pertence a otro programa</li>';
					}else{
	...
	public function pago($tipopago,$idpostulacionitem){
		...
		$parts = explode(',',base64_decode($idpostulacionitem));
		asort($parts);		
		$idpostulacionitem = base64_encode(end($parts));
		...
		switch($tipopago){
			...
			case 1441 :
				...
				$data['bancos'] = $this->Ta_metadatodetalle->get_bancos();			
				break;
			//Por facturar empresa
			case 3811:
			    //$idpostulacionitem = base64_encode(306075);
				$page = 'pago_porfacturarempresa';
				$title = 'Pago por Facturar';
			...
			case 1442:
				...
				//echo base64_decode($idpostulacionitem);
				if(count($data['pagados']) < 1){
					$data['rango'] = 1;	
				}else{
					$data['rango'] = count($data['pagados']);	
	...
	public function registrarpago($tipopago,$idpostulacionitem){		
		...
		if($estado == 3 OR $estado == 12 OR $estado == 9){
			...
			switch($tipopago){
				...
				$this->form_validation->set_rules('factura_otic','factura otic','trim');
				$this->form_validation->set_rules('orden_otic','orden otic','trim');		
				$this->form_validation->set_rules('monto_otic','monto','trim|callback_onlynumbers|numeric|greater_than[0]');						
				break;
				case 1442 :
				...
				if(is_numeric($tipopago)){
					...
					switch($tipopago){
						...
						case 3811:
							...
							$this->Tz_pagoporfacturar->update_pago_porfacturar_empresa(base64_decode($idpostulacionitem),
							$monto_empresa,
							$descripcion_empresa,
							$factura_empresa,
							$fecha_empresa,
						...
						case 5555:
							...
							$this->Tz_pagoporfacturar->update_pago_porfacturar_otic(base64_decode($idpostulacionitem),
							$monto_otic,
							$descripcion_otic,
							$factura_otic,
							$fecha_otic,
	...
	public function detallepago($idpostulacionitem){
		...
		if(count($grupos) > 0){
			...
			foreach($grupos as $value){
				...
				$ids_postulacionitems[] = $value['IdPostulacionItem'];
				unset($detalle);
			}
		}else{
			...
			'solo_cuota' => $detalle->solo_cuota,
			'IDSeccion' => $detalle->IDSeccion);
			$ids_postulacionitems[] = $idpostulacionitem;
			unset($detalle);
		}
		
		$res = $this->Tz_postulacionitem_tipodepago->get_formaspago_porpostulacionitem(implode(',',$ids_postulacionitems));
		...
		foreach($res as $value){
			...
			if($value['IDTipoPago']){			
				...
				$res2 = $this->Tz_postulacionitem_tipodepago->get_detalle_delpago(implode(',',$ids_postulacionitems),$value['IDTipoPago']);		
				$data['pagos_padre'][$x]['detalle'] = $res2;
