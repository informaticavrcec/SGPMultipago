...
class Asignarboletasfacturas extends MY_Controller {	
	...
	function index(){
		...
		if($this->form_validation->run() == TRUE){
			...
			$this->load->model(array('Ta_usrio','Tz_grupospago'));
			...
			if(is_numeric($data['id_usrio'])){				
				...
				$listado = $this->Tz_postulacionitem->get_actividades_estado($data['id_usrio'],'3,12,4');
				$c = 0 ;
				foreach($listado as $value){					
					...
					$grupos = $this->Tz_grupospago->get_grupo_actividad($value['IDPostulacionItem']);
					if(count($grupos) > 0){
						
						$data['actividades'][$c]['Nombre_Curso'] = '<ul>';
						foreach($grupos as $actividad){
							$data['actividades'][$c]['Nombre_Curso'] .= '<li>'.$actividad['Nombre_Curso'].' <strong>'.$value['es_admision'].'</strong></li>';
							$data['actividades'][$c]['ValorNuevo'] += $actividad['ValorNuevo'];
						}
						$data['actividades'][$c]['Nombre_Curso'] .= '</ul>';
					}else{
						$data['actividades'][$c]['Nombre_Curso'] = $value['Nombre_Curso'].' <strong>'.$value['es_admision'].'</strong>';
						$data['actividades'][$c]['ValorNuevo'] = $value['ValorNuevo'];
					}					
					
					$data['actividades'][$c]['IDPostulacionItem'] = $value['IDPostulacionItem'];
