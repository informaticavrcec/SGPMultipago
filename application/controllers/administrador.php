...
class Administrador extends MY_Controller {
	...
	function agregarusuario(){
		...
		$this->form_validation->set_rules('rut','rut usuario','trim|required');		
		if($this->form_validation->run() === TRUE){
			$this->load->model(array('Ta_usrio'));
			$data['encontrados'] = $this->Ta_usrio->buscar_usuario($this->input->post('rut'));
			$seleccion = $this->input->post('add');
			if(count($seleccion) > 0){				
				foreach($seleccion as $valor){
					if(is_numeric($valor)){
						$data['error'][$valor] = $this->Tz_permisos->agregar_usuarios($valor,$this->session->userdata('id_usuario'));	
					}
				}				
			}			
		}else{
		...
		$this->load->view('administradores/agregar',$data);		
	}
	...
	function fichausuario($idt_usrio){
		...
		if($this->form_validation->run() == TRUE){
			...
			$data['error_rut'] = $this->Ta_usrio->update_rut($this->input->post('rut'),$idt_usrio);
